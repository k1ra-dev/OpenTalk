...
def parse_sources(data: str) -> list[tuple[str, str]]:
    """Parse pactl JSON and return usable physical input sources."""
    decoded = json.loads(data)
    if not isinstance(decoded, list):
        raise ValueError("pactl-Ausgabe enthält keine Quellenliste")
    result = []
    for item in decoded:
...
def list_sources() -> list[tuple[str, str]]:
    from opentalk import recorder_dependency, subprocess_options
    ...
    if platform.system() == "Windows":
        try:
            ...
            raise RuntimeError(
                "Mikrofone konnten nicht gelesen werden (FFmpeg/DirectShow prüfen)."
            ) from exc
    try:
        response = subprocess.run(
        ...
        raise RuntimeError(
            "Mikrofone konnten nicht gelesen werden (pactl / PipeWire prüfen)."
        ) from exc
